@extends('layout.app')
@section('title','Simanja : Edit Host Sever')
@section('content')
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4>Edit Host Servers</h4>
                        </div>
                        <div class="card-body" >
                            <form METHOD="POST" action="/host/{{$host->id_host}}">
                                @method('PUT')
                                @csrf
                                <div class="form-group row mb-4">
                                    <label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Host Name</label>
                                    <div class="col-sm-12 col-md-7">
                                        <input type="text" class="form-control @error('host_name') is-invalid @enderror" name="host_name" value="{{$host->host_name}}" placeholder="Format Nama Server : UPT TIK  HOST">
                                        @error('host_name')
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <input type="text" class="form-control @error('host_ip') is-invalid @enderror" name="host_ip"  value={{$host->host_ip}} placeholder="Inputkan IPv4 yang Valid " >
                                        @error('host_ip')
                                        <div class="invalid-feedback">
                                            {{$message}}
                                        </div>
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <input type="text" class="form-control @error('host_auth') is-invalid @enderror" name="host_auth"  value="{{$host->host_auth}}" placeholder=" User : Password ">
                                        @error('host_auth')
                                        <div class="invalid-feedback">
                                            {{$message}}
                                        </div>
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select class="form-control selectric @error('id_os')  is-invalid @enderror" name="id_os" id="id_os" >
                                            @foreach($os as $oss)
                                                <option @selected($oss->id_os == $host->id_rack)
                                                        value="{{ $oss->id_os }}">{{ $oss->os_name }}</option>
                                            @endforeach
                                        </select>
                                        @error('id_os')
                                        <div class="invalid-feedback">
...
{{--                                                <option value="{{$devices->id_srv}}" @selected(old('id_srv') == $devices->id_srv)>{{$devices->srv_name}} : {{$devices->srv_ip}} : {{$devices->rack->rack_number}}</option>--}}
                                                <option @selected($devices->id_srv == $host->id_srv)
                                                        value="{{ $devices->id_srv }}">{{$devices->srv_name}} : {{$devices->srv_ip}} : {{$devices->rack->rack_number}}</option>
                                            @endforeach
                                        </select>
                                        @error('id_srv')
                                        <div class="invalid-feedback">
                                ...
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select class="form-control  @error('status') is-invalid @enderror selectric" name="status" id="status">
                                            ...
                                            <option value="Deactivate" @selected($host->status == 'Deactivate')>Deactivate</option>
                                        </select>
                                        @error('status')
                                        <div class="invalid-feedback">
                                            {{$message}}
